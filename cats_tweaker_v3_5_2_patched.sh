...
#     ╚═════╝╚═╝  ╚═╝   ╚═╝   ╚══════╝       ╚═╝    ╚══╝╚══╝ ╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝
#
#                       「  v3.5.2 - M4 PRO FINAL EDITION  」
#                         1930-2026 + NES→PS5 TOOLKIT
#                            by Flames / Team Flames 🐱
#                    [PATCHED: DASM, z88dk, rgbasm, mips64 fixes]
#
#===============================================================================
...
if command -v mips64-elf-gcc &>/dev/null; then
    ...
    # Method 1: Try libdragon toolchain (requires Docker)
    if command -v docker &>/dev/null && command -v libdragon &>/dev/null; then
        info "Trying libdragon toolchain via Docker..."
        if libdragon init 2>>"$LOG" && docker images | grep -q libdragon; then
    ...
    # Method 2: Try pre-built from crashoverride (common N64 toolchain source)
    if [[ -z "$MIPS_OK" ]]; then
        info "Trying pre-built toolchain..."
        $IS_APPLE_SILICON && MIPS_ARCH="arm64" || MIPS_ARCH="x86_64"
        
        # Try n64chain releases
        if dl "https://github.com/tj90241/n64chain/releases/download/13.2.0/n64chain-macos-${MIPS_ARCH}.tar.gz" mips.tar.gz 2>/dev/null; then
            tar xzf mips.tar.gz -C "$COMPILERS/mips-toolchain" --strip-components=1 >> "$LOG" 2>&1
            rm -f mips.tar.gz
            [[ -x "$COMPILERS/mips-toolchain/bin/mips64-elf-gcc" ]] && ok "mips64-elf-gcc (n64chain)" && MIPS_OK=1
        fi
    fi
    
    # Method 3: Build from source (fallback)
    if [[ -z "$MIPS_OK" ]]; then
        warn "Pre-built unavailable, creating build script..."
        cat > build_mips.sh << 'MIPS'
#!/bin/bash
set -e
echo "🔧 Building MIPS64 toolchain (this takes 20-40 minutes)..."
...
mkdir -p build && cd build
../configure --target=$TARGET --prefix=$PREFIX --disable-nls --enable-languages=c,c++ --without-headers --with-newlib --disable-libssp --disable-multilib --disable-threads --disable-libgomp
make -j$J all-gcc all-target-libgcc
make install-gcc install-target-libgcc

echo "✅ Done! Toolchain installed to $PREFIX/bin"
...
echo "  🐱 CAT'S TWEAKER v3.5.2 loaded!"
ENV
chmod +x "$INSTALL_DIR/env.sh"; ok "env.sh"
add_path ""; add_path "# Cat's Tweaker v3.5.2"; add_path "source \"$INSTALL_DIR/env.sh\" 2>/dev/null"
...
echo ""
printf "${G}╔═════════════════════════════════════════════════════════════════════╗${RST}\n"
printf "${G}║${RST}  ${W}✨ CAT'S TWEAKER v3.5.2 COMPLETE! ✨${RST}                             ${G}║${RST}\n"
