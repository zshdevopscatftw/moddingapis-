...
GIT_TERMINAL_PROMPT=0 "$BREW" install z88dk >> "$LOG" 2>&1 && ok "z88dk" || warn "z88dk"
...
[[ -x "$SDKS/atari/dasm" ]] && ok "DASM" || {
    dl "https://sourceforge.net/projects/dasm-dillon/files/dasm-dillon/2.20.14.1/dasm-2.20.14.1-osx-x64.tar.gz/download" dasm.tar.gz && {
        tar xzf dasm.tar.gz >> "$LOG" 2>&1; rm -f dasm.tar.gz; chmod +x dasm* 2>/dev/null; ok "DASM"
    } || warn "DASM"
}
...
echo "  🐱 CAT'S TWEAKER v3.5 loaded!"
ENV
chmod +x "$INSTALL_DIR/env.sh"; ok "env.sh"
add_path ""; add_path "# Cat's Tweaker v3.5"; add_path "source \"$INSTALL_DIR/env.sh\" 2>/dev/null"
...
command -v rgbasm &>/dev/null && ok "rgbasm"
...
echo ""
printf "${G}╔═════════════════════════════════════════════════════════════════════╗${RST}\n"
printf "${G}║${RST}  ${W}✨ CAT'S TWEAKER v3.5 COMPLETE! ✨${RST}                               ${G}║${RST}\n"
